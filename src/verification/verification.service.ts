...
import { IVerification, VerificationDto } from './verification';
    ...
    async sendVerificationEmail(emailAddress: string){
        ...
        try {
            ...
            try {
                ...
                this.logger.error(`There was an error adding the verification to the database: ${error}`);
                throw new HttpException(`There was an error adding the verification to the database: ${error}`, HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } catch (error) {
            this.logger.error(`There was an error sending an email: ${error}`);
            throw new HttpException(`There was an error sending an email: ${error}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
