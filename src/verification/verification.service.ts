...
export class VerificationService {
    ...
    try {
      ...
      try {
        ...
        if (verification.code === code) {
          ...
          this.logger.error(
            `Verification error for email: ${email}, code is invalid`
          )
  ...
  private async addVerificationCode(
    ...
    try {
      ...
      this.logger.error(
        `Could not find user by email: ${email}, with error: ${error}`
      )
      throw new HttpException(
        'There was an error producing the verification code.',
        HttpStatus.BAD_REQUEST
      )
    }
  }
